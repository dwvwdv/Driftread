-- Enable Row Level Security on public-facing catalog tables.
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
     WHERE schemaname = 'driftread' AND tablename = 'feeds' AND policyname = 'feeds_public_read'
  ) THEN
    CREATE POLICY feeds_public_read ON driftread.feeds
      FOR SELECT TO anon, authenticated USING (TRUE);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
     WHERE schemaname = 'driftread' AND tablename = 'articles' AND policyname = 'articles_public_read'
  ) THEN
    CREATE POLICY articles_public_read ON driftread.articles
      FOR SELECT TO anon, authenticated USING (TRUE);
  END IF;
END $$;
