...
@pytest.mark.asyncio
async def test_resolve_pinned_ips_returns_the_resolved_addresses():
    with patch(
        "services.feed_discovery.socket.getaddrinfo",
        return_value=_fake_addrinfo("93.184.216.34"),
    ):
        assert await _resolve_pinned_ips("example.com") == ["93.184.216.34"]


@pytest.mark.asyncio
async def test_resolve_pinned_ips_returns_every_address_in_order():
    ...
    with patch(
        ...
        assert await _resolve_pinned_ips("dualstack.example.com") == [
            "2606:4700:4700::1111",
            "93.184.216.34",
        ]
...
@pytest.mark.asyncio
async def test_resolve_pinned_ips_rejects_private_address():
    with patch(
        "services.feed_discovery.socket.getaddrinfo",
        return_value=_fake_addrinfo("169.254.169.254"),
    ):
        with pytest.raises(DiscoveryError):
            await _resolve_pinned_ips("evil.example.com")


@pytest.mark.asyncio
async def test_resolve_pinned_ips_rejects_host_if_any_of_several_addresses_unsafe():
    ...
    with patch(
        ...
        with pytest.raises(DiscoveryError):
            await _resolve_pinned_ips("multi-record.example.com")


@pytest.mark.asyncio
async def test_resolve_pinned_ips_propagates_dns_failure():
