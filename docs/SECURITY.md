...
禮貌延遲涵蓋所有三個階段：探測由 `discover_feeds()` 自己排（含 robots→首頁那一段），收割與
目錄則由 gate 的 `pace=True` 負責 —— gate 正好在每個請求（含 redirect hop）之前被呼叫，所以
把睡眠放在那裡最不容易漏。另外，同一個站一次只有一條請求流：`probe_due()` 除了 semaphore 還
有一組以 `site_key` 為鍵的鎖，否則同一發布者的多個待探測列會並行、延遲互相重疊。
